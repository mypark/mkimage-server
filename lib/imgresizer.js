                                        ...
                                        cv.readImage(downloaded_image, function(err, im){
                                             require('./facedetect').getFacialGravity(im, function(err, gravity){
                                                if (gravity)
                                                {
                                                    ...
                                                    // options.origW = origWidth;
                                                    // options.origH = origHeight;
                                                    options.y = options.x = 0;
                                                    if (width > height){ //finagle the height pan otherwise we adjust the horizontal pan

                                                        //var deltaY = gravity.y - origHeight/2;
                                                        //var maxDeltaY = (origHeight - height)/2;

                                                        var maxDeltaY = (origHeight - height);

                                                        // console.log('gravity:' + gravity.y);
                                                        // console.log('deltaY:' + deltaY);
                                                        // if (Math.abs(deltaY) > maxDeltaY)
                                                        //     options.y = deltaY > 0 ? maxDeltaY : -maxDeltaY;
                                                        // else
                                                        //     options.y = deltaY;


                                                        options.y = gravity.y > maxDeltaY ? maxDeltaY : gravity.y;
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        var deltaX = gravity.x - origWidth/2;
                                                        var maxDeltaX = (origWidth - width)/2;

                                                        if (Math.abs(deltaX) > maxDeltaX)
                                                            options.x = deltaX > 0 ? maxDeltaX : -maxDeltaX;
                                                        else
                                                            options.x = deltaX;
                                                    }
