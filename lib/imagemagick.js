...
function im_crop(options, force, callback)
{
    ...
    // var  imcmd = sprintf('convert %s -resize %dx%d^ %s', options.src, options.width, options.height, options.dst);

    function getSignedNumberString(theNumber)
    {
        if(theNumber >= 0){
            return "+" + theNumber;
        }else{
    ...
    var  imcmd = sprintf('convert %s -resize %dx%d^ -crop %sx%s%s%s^ -gravity %s -quality %d %s', options.src, options.width, options.height, options.width, options.height, getSignedNumberString(options.x), getSignedNumberString(options.y), options.gravity, options.quality, options.dst);
    
    //var imcmd = sprintf('convert %s -thumbnail %sx%s^ -gravity %s  -quality %d -extent %sx%s %s',options.src, options.width, options.height, options.gravity, options.quality,options.x, options.y, options.dst);
