...
function im_crop(options, force, callback)
{
    ...
    var  imcmd = sprintf('convert %s -resize %dx%d^ -crop %sx%s+%d+%d^ -gravity %s -quality %d %s', options.src, options.width, options.height, options.width, options.height, options.x, options.y, options.gravity, options.quality, options.dst);
